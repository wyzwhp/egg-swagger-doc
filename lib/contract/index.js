  ...
  for (let name of names) {
    ...
    if (stat.isFile() && ['.js', '.ts'].indexOf(path.extname(filepath)) !== -1) {
      ...
      for (let object in def) {
        let path = filepath.replace(/\.(js|ts)/g, '').split(/app[\\|/]contract[\\|/]/)[1].replace(/[\\|/]/g, '.');
        path = path.replace(/([^-])(?:-+([^-]))/g, function($0, $1, $2) {
          return $1 + $2.toUpperCase();
        });

        _.map(def[object], (val, key) => {
          def[object][key] = Object.assign({
            required: false,
            convertType: val.type,
          }, val);
        });

        CONTRACT[`${path}.${object}`] = CONTRACT[object] = {
...
function buildValidateRule(source) {
  ...
  for (let object in source) {
    ...
    for (let field in def) {
      ...
      // #region 对array数组的处理
      if (def[field].type === 'array' && !def[field].rule) {
