...
function generateSecurity(block, securitys, swagger) {
  ...
  for (let security of securitys) {
    if (block.indexOf(`@${security}`) > -1 || (swagger.securities || []).includes(security)) {
      let securityItem = {};
    ...
    for (let response of responses) {
      ...
      if (response[2]) {
        if (response[2].startsWith('array')) {
          ...
          let items = {};
          let itemsType = response[2].substring(6, response[2].length - 1);
...
function generateParameters(request, routers, definitions) {
  ...
  } else if (definitions.hasOwnProperty(request[2])) {
    const parameters = [];
    lodash.map(definitions[request[2]].properties, (val, key) => {
      const types = ['boolean', 'integer', 'number', 'string'];
      ...
      parameter.required = definitions[request[2]].required.includes(key);
      val.hasOwnProperty('example') && (parameter.example = val.example);
      val.hasOwnProperty('description') && (parameter.description = val.description);
      val.hasOwnProperty('enum') && (parameter.enum = val.enum);
